...
@ActiveProfiles("test")
@Tag("test")
public class EmployeeCheckedDelegateTest {
	...
	@InjectMocks
	EmployeeCheckedDelegate employeeCheckedDelegate;
	...
	public void testJavaServiceDelegation() throws Exception {
		employeeCheckedDelegate.notify(delegateTask);
		verify(delegateTask).setVariable(argThat((event) -> event.equals("currentState")),
				same(EmployeeState.APPROVED.toString()));
		verify(delegateTask).setVariable(argThat((event) -> event.equals("nextEvent")),
				same(TaskEvent.ACTIVATED.toString()));
		verify(employeeService, times(1)).addEmployeeEvent(notNull(), same(EmployeeState.APPROVED));

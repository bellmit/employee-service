...
public class EmployeeIntegrationTest {
	...
	void GivenEmployeeWhenChangeStateToActivateReturnSuccess() throws Exception {
		...
		changeEmployeeStateSucccess(responseModel, TaskEvent.ADDED, EmployeeState.INCHECK);
		changeEmployeeStateSucccess(responseModel, TaskEvent.APPROVED, EmployeeState.APPROVED);
		changeEmployeeStateSucccess(responseModel, TaskEvent.ACTIVATED, EmployeeState.ACTIVE);
	...
	void GivenEmployeeWhenChangeStateAfterActiveThrowInvalidEvent() throws Exception {
		...
		mockMvc.perform(patch("/employee/" + empCode).accept((MediaType.APPLICATION_JSON))
				.contentType(MediaType.APPLICATION_JSON_VALUE).content(asJsonString(createStateEvent(TaskEvent.ADDED)))).andDo(print())
		...
		mockMvc.perform(patch("/employee/" + empCode).accept((MediaType.APPLICATION_JSON))
				.contentType(MediaType.APPLICATION_JSON_VALUE).content(asJsonString(createStateEvent(TaskEvent.APPROVED)))).andDo(print())
				// Validate the response code and content type
				.andExpect(status().isNotFound()).andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
				// Validate the returned fields
				.andExpect(jsonPath("$['message']", IsAnything.anything()));
		...
		mockMvc.perform(patch("/employee/" + empCode).accept((MediaType.APPLICATION_JSON))
				.contentType(MediaType.APPLICATION_JSON_VALUE).content(asJsonString(createStateEvent(TaskEvent.ACTIVATED)))).andDo(print())
				// Validate the response code and content type
				.andExpect(status().isNotFound()).andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
				// Validate the returned fields
				.andExpect(jsonPath("$['message']", IsAnything.anything()));
	...
	private void changeEmployeeStateSucccess(ResponseModel responseModel, TaskEvent event, EmployeeState nextState) throws Exception {
		mockMvc.perform(patch("/employee/" + responseModel.getEmpCode()).accept((MediaType.APPLICATION_JSON))
	...
	private StateEventRequest createStateEvent(TaskEvent employeeEvent) {

...
public class EmployeeRepositoryTest {
	...
	@Test
	@Transactional
	@DisplayName("Get find employee by businessKey Should return Employee")
	public void WhenFindByBusinessKeyThenReturnSuccess() {
		Employee employee = creatEmployeeEntity();
		employeeRepository.save(employee);
		employee = employeeRepository.findByBusinessKey(employee.getEmpCode());
		assertThat(employee.getId()).isNotNull();
		assertThat(employee.getId()).isGreaterThan(0);
	}
	...
	public void clearDatabase() {
		template.update("DELETE from employee_event");
